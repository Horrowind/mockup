...
u8 r65816_mapper_read(r65816_mapper_t* mapper, u32 addr) {
    u8* data_ptr = r65816_mapper_ptr(mapper, addr);
    if(data_ptr) return *data_ptr;
    return 0;
}

void r65816_mapper_write(r65816_mapper_t* mapper, u32 addr, u8 data) {
    u8* data_ptr = r65816_mapper_ptr(mapper, addr);
...
u8* r65816_mapper_ptr(r65816_mapper_t* mapper, u32 full_addr) {
    ...
    for(int i = 0; i < mapper->num_entries; i++) {
        r65816_mapper_entry_t* entry = &mapper->entries[i];
        if(entry->bank_low <= bank && bank <= entry->bank_high
           && entry->addr_low <= addr && addr <= entry->addr_high) {
            u32 offset = r65816_mapper_reduce(full_addr, entry->mask);
            if(entry->size) offset = entry->base + r65816_mapper_mirror(offset, entry->size - entry->base);
...
r65816_mapper_function_triple_t r65816_mapper_create_function_triple(r65816_mapper_t* mapper) {
    ...
    LLVMValueRef reduce_function = LLVMGetNamedFunction(mapper_module, "r65816_mapper_reduce");
    LLVMValueRef mirror_function = LLVMGetNamedFunction(mapper_module, "r65816_mapper_mirror");
    ...
    for(int i = 0; i < 3; i++) {
        ...
        for(int j = 0; j < mapper->num_entries; j++) {
            r65816_mapper_entry_t* entry = &mapper->entries[j];
    ...
    r65816_mapper_function_triple_t triple = { 0 };
    LLVMOrcGetSymbolAddress(orc_ref, (LLVMOrcTargetAddress*)&triple.read,  "mapper_read");
    LLVMOrcGetSymbolAddress(orc_ref, (LLVMOrcTargetAddress*)&triple.write, "mapper_write");
    LLVMOrcGetSymbolAddress(orc_ref, (LLVMOrcTargetAddress*)&triple.ptr,   "mapper_ptr");
    return triple;
}
