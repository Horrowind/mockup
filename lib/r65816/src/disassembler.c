...
uint32_t r65816_decode(r65816_cpu_t* cpu, uint8_t offset_type, uint32_t addr) {
  ...
  switch(offset_type) {
    ...
    case OPTYPE_IDP:
      ...
      r = (cpu->regs.db << 16) + r65816_dreadw(cpu, addr);
      break;
    case OPTYPE_IDPX:
    ...
    case OPTYPE_IDPY:
      addr = (cpu->regs.d.w + (addr & 0xffff)) & 0xffff;
      r = (cpu->regs.db << 16) + r65816_dreadw(cpu, addr) + cpu->regs.y.w;
      break;
    case OPTYPE_ILDP:
      addr = (cpu->regs.d.w + (addr & 0xffff)) & 0xffff;
      r = r65816_dreadl(cpu, addr);
      break;
    case OPTYPE_ILDPY:
      addr = (cpu->regs.d.w + (addr & 0xffff)) & 0xffff;
      r = r65816_dreadl(cpu, addr) + cpu->regs.y.w;
      break;
    case OPTYPE_ADDR:
