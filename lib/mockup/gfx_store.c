...
void gfx_store_init(GFXStore* gfx_store, Wdc65816Mapper* rom, Arena* arena) {
    ...
    for(int i = 0; i < 0x32; i++) {
        ...
        u8 bank = wdc65816_mapper_read(rom, 0x00B9F6 + i);
        //u16 addr = (rom->banks[0][0xB9C4 + i] << 8) | rom->banks[0][0xB992 + i];
        u16 addr = (wdc65816_mapper_read(rom, 0x00B9C4 + i) << 8) |
            wdc65816_mapper_read(rom, 0x00B992 + i);
        //gfx_store->pages[i].length = decode_lz2_get_size(rom->banks[bank] + addr);
        gfx_store->pages[i].length =
            decode_lz2_get_size(wdc65816_mapper_ptr(rom, (bank << 16)  + addr));
    ...
    for(int i = 0; i < 0x32; i++) {
        ...
        sum_length += gfx_store->pages[i].length;
        decode_lz2(wdc65816_mapper_ptr(rom, bank * 0x10000 + addr),
    ...
    decode_lz2(wdc65816_mapper_ptr(rom, addr_gfx32_sfc), gfx_store->pages[0x32].data);
    const int addr_gfx33_sfc = 0x08BFC0;
    decode_lz2(wdc65816_mapper_ptr(rom, addr_gfx33_sfc), gfx_store->pages[0x33].data);
}
