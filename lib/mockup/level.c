...
void level_load_palette(level_t* l) {
    ...
    l->cpu->ram[0x66] = l->cpu->rom[0x02E000 + 3 * m_levelnum + 1];
    l->cpu->ram[0x67] = l->cpu->rom[0x02E000 + 3 * m_levelnum + 2];
    r65816_run(l->cpu, 0x0583AC, 0x0583B8);
    // Run the palette loading routine.
    
    //m_cpu.run(0x00ABED, 0x00ACEC);
    uint16_t paletteSNES[256];
    memcpy(paletteSNES, l->cpu->ram + 0x0703, 512);
    
    for(int i = 0; i < 256; i++) {
	l->palette[i] = i % 16 ? convert_color(paletteSNES[i]) : 0;
    }
}
