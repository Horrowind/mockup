...
//#define opXF( id, name, fn   ) op_table[table_EM + id] = op_table[table_MX + id] = op_table[table_mX + id] = &op_##name##_b<&op_##fn##_b>; op_table[table_Mx + id] = op_table[table_mx + id] = &op_##name##_w<&op_##fn##_w>
    ...
    opM(0x74, write_dpr, Z, X);
    opM(0x75, read_dpr_adc);
    opM(0x76, adjust_dpx, ror);
    ...
    opA(0x8b, phb);
    opX(0x8c, write_addr, Y);
    opM(0x8d, write_addr, A);
    opX(0x8e, write_addr, X);
    opM(0x8f, write_longr, Z);
    opA(0x90, branch_bcc);
    opM(0x91, sta_idpy);
    opM(0x92, sta_idp);
    opM(0x93, sta_isry);
    opX(0x94, write_dpr, Y, X);
    opM(0x95, write_dpr, A, X);
    opX(0x96, write_dpr, X, Y);
    opM(0x97, sta_ildpy);
    opM(0x98, transfer_tya);
    opM(0x99, write_addrr, A, Y);
    opE(0x9a, txs);
    opX(0x9b, transfer_txy);
    opM(0x9c, write_addr, Z);
    opM(0x9d, write_addrr, A, X);
    opM(0x9e, write_addrr, Z, X);
    opM(0x9f, write_longr, X);
    opX(0xa0, read_const_ldy);
    opM(0xa1, read_idpx, lda);
