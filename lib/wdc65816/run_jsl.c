...
#define opcode(funname)                                                 \
    ...
    if((cpu->regs.s.w != 0x1FF || wdc65816_cpu_read(cpu, cpu->regs.pc.d) != 0x6B) && !cpu->stop_execution) return;
#else
#define opcode(funname)                                                 \
    funname(cpu);                                                       \
    cpu->stop_execution |= wdc65816_breakpoint_list_is_hit(&cpu->breakpoints_exec, cpu->regs.pc.d); \
...
#define opE(id, name)                               \
    EM_##id:                                        \
        opcode(wdc65816_op_##name##_e);               \
        goto *jump_table_EM[wdc65816_op_readpc(cpu)]; \
    MX_##id:                                        \
        opcode(wdc65816_op_##name##_n);               \
        goto *jump_table_MX[wdc65816_op_readpc(cpu)]; \
    Mx_##id:                                        \
        opcode(wdc65816_op_##name##_n);               \
        goto *jump_table_Mx[wdc65816_op_readpc(cpu)]; \
