...
#define wdc65816_op_move_gen(name, adjust)                                      \
    static void wdc65816_op_move_##name##_b(Wdc65816Cpu* cpu) {                 \
        ...
        if(cpu->regs.a.w--) cpu->regs.pc.w -= 3;                                \
                                                                                ...
                                                                                \
    static void wdc65816_op_move_##name##_w(Wdc65816Cpu* cpu) {                 \
        cpu->dp = wdc65816_op_readpc(cpu);                                      \
        cpu->sp = wdc65816_op_readpc(cpu);                                      \
        cpu->regs.db = cpu->dp;                                                 \
        cpu->rd.l = wdc65816_op_readlong(cpu, (cpu->sp << 16) | cpu->regs.x.w); \
...
#define wdc65816_op_interrupt_gen(name, vectorE, vectorN)                       \
    static void wdc65816_op_interrupt_##name##_e(Wdc65816Cpu* cpu) {            \
        wdc65816_op_readpc(cpu);                                                \
        ...
        cpu->regs.pc.w = cpu->rd.w;                                             \
                                                                                ...
                                                                                \
    static void wdc65816_op_interrupt_##name##_n(Wdc65816Cpu* cpu) {            \
        wdc65816_op_readpc(cpu);                                                \
...
#define wdc65816_op_flag_gen(name, mask, value)                                 \
    static void wdc65816_op_flag_##name(Wdc65816Cpu* cpu) {                     \
...
#define wdc65816_op_pflag_gen(name, mode)                                       \
    static void wdc65816_op_pflag_##name##_e(Wdc65816Cpu* cpu) {                \
        ...
        if(cpu->regs.p.x) {                                                     \
            ...
            cpu->regs.y.h = 0x00;                                               \
                                                                                ...
                                                                                \
    static void wdc65816_op_pflag_##name##_n(Wdc65816Cpu* cpu) {                \
        cpu->rd.l = wdc65816_op_readpc(cpu);                                    \
        cpu->regs.p.b = mode;                                                   \
        if(cpu->regs.p.x) {                                                     \
            cpu->regs.x.h = 0x00;                                               \
...
#if 1
#define wdc65816_op_transfer_b_gen(name, from, to)                              \
    static void wdc65816_op_transfer_##name##_b(Wdc65816Cpu* cpu) {             \
...
#define wdc65816_op_transfer_w_gen(name, from, to)                              \
    static void wdc65816_op_transfer_##name##_w(Wdc65816Cpu* cpu) {             \
...
#if 1
wdc65816_op_transfer_w_gen(tsc, S, A);
wdc65816_op_transfer_w_gen(tcd, A, D);
wdc65816_op_transfer_w_gen(tdc, D, A);
#endif

wdc65816_op_transfer_b_gen(txa, X, A);
wdc65816_op_transfer_b_gen(tya, Y, A);
wdc65816_op_transfer_b_gen(txy, X, Y);
wdc65816_op_transfer_b_gen(tay, A, Y);
wdc65816_op_transfer_b_gen(tax, A, X);
wdc65816_op_transfer_b_gen(tyx, Y, X);

wdc65816_op_transfer_w_gen(txa, X, A);
wdc65816_op_transfer_w_gen(tya, Y, A);
wdc65816_op_transfer_w_gen(txy, X, Y);
wdc65816_op_transfer_w_gen(tay, A, Y);
wdc65816_op_transfer_w_gen(tax, A, X);
wdc65816_op_transfer_w_gen(tyx, Y, X);
#undef wdc65816_op_transfer_b_gen
#undef wdc65816_op_transfer_w_gen
...
#define wdc65816_op_push_gen(name, n)                                           \
    static void wdc65816_op_push_##name##_b(Wdc65816Cpu* cpu) {                 \
        wdc65816_op_writestack(cpu, cpu->regs.r[n].l);                          \
    }                                                                           \
                                                                                \
    static void wdc65816_op_push_##name##_w(Wdc65816Cpu* cpu) {                 \
