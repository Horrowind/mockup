...
#define wdc65816_op_move_gen(name, adjust)                                \
    void wdc65816_op_move_##name##_b(wdc65816_cpu_t* cpu) {                 \
        ...
        if(cpu->regs.a.w--) cpu->regs.pc.w -= 3;                        \
                                                                        ...
                                                                        \
    void wdc65816_op_move_##name##_w(wdc65816_cpu_t* cpu) {                 \
        cpu->dp = wdc65816_op_readpc(cpu);                                \
        cpu->sp = wdc65816_op_readpc(cpu);                                \
        cpu->regs.db = cpu->dp;                                         \
        cpu->rd.l = wdc65816_op_readlong(cpu, (cpu->sp << 16) | cpu->regs.x.w); \
...
#define wdc65816_op_interrupt_gen(name, vectorE, vectorN)         \
    void wdc65816_op_interrupt_##name##_e(wdc65816_cpu_t* cpu) {    \
        wdc65816_op_readpc(cpu);                                  \
        ...
        cpu->regs.pc.w = cpu->rd.w;                             \
                                                                ...
                                                                \
    void wdc65816_op_interrupt_##name##_n(wdc65816_cpu_t* cpu) {    \
        wdc65816_op_readpc(cpu);                                  \
...
#define wdc65816_op_flag_gen(name, mask, value)               \
    void wdc65816_op_flag_##name(wdc65816_cpu_t* cpu) {         \
...
#define wdc65816_op_pflag_gen(name, mode)                     \
    void wdc65816_op_pflag_##name##_e(wdc65816_cpu_t* cpu) {    \
        ...
        if(cpu->regs.p.x) {                                 \
            ...
            cpu->regs.y.h = 0x00;                           \
                                                            ...
                                                            \
    void wdc65816_op_pflag_##name##_n(wdc65816_cpu_t* cpu) {    \
        cpu->rd.l = wdc65816_op_readpc(cpu);                  \
        cpu->regs.p.b = mode;                               \
        if(cpu->regs.p.x) {                                 \
            cpu->regs.x.h = 0x00;                           \
...
#define wdc65816_op_transfer_gen(name, from, to)              \
    void wdc65816_op_transfer_##name##_b(wdc65816_cpu_t* cpu) { \
        cpu->regs.r[to].l = cpu->regs.r[from].l;            \
        cpu->regs.p.n = 1 && (cpu->regs.r[to].l & 0x80);    \
        cpu->regs.p.z = (cpu->regs.r[to].l == 0);           \
    }                                                       \
                                                            \
    void wdc65816_op_transfer_##name##_w(wdc65816_cpu_t* cpu) { \
...
#define wdc65816_op_push_gen(name, n)                                     \
    void wdc65816_op_push_##name##_b(wdc65816_cpu_t* cpu) {                 \
        wdc65816_op_writestack(cpu, cpu->regs.r[n].l);                    \
    }                                                                   \
                                                                        \
    void wdc65816_op_push_##name##_w(wdc65816_cpu_t* cpu) {                 \
