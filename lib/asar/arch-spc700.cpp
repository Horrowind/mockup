...
#define isop(test) (!strcasecmp(op, test))
...
#define isop(str) (!strcasecmp(word[0], str))
#define isam(str) (!strcasecmp(arg[0], str))
...
#define match(left, right) if (ismatch(left, right))
			...
			if (bitmatch(word[0], op, arg[0], math, bits))
			{
				...
				else if (!strcasecmp(op, "set")) write1(0x02|(bits<<5));
				else if (!strcasecmp(op, "clr")) write1(0x12|(bits<<5));
				else return false;
				int num=getnum(math);
...
#undef match
		...
		if (numwords==2)
		{
#define iscc(str1, str2) (!strcasecmp(arg[0], str1) && !strcasecmp(arg[1], str2))
#define iscv(str1, left2, right2) (!strcasecmp(arg[0], str1) && matchandwrite(arg[1], left2, right2, s2))
#define isvc(left1, right1, str2) (matchandwrite(arg[0], left1, right1, s1) && !strcasecmp(arg[1], str2))
...
#define w1r(opcode, math1, math2) do { int len=getlen(math2); int num=getnum(math2); int pos=(len==1)?num:num-(snespos+3); \
			...
			int bits;
#define isop(test) (!strcasecmp(op, test))
			if (!strcasecmp(arg[0], "c") && bitmatch(word[0], op, arg[1], math, bits))
			{
				if (assinglebitwithc(op, math, bits)) return true;
			}
			if (bitmatch(word[0], op, arg[0], s1, bits))
			{
				if (isop("mov") && !strcasecmp(arg[1], "c"))
				{
					int num=getnum(s1);
					if (num>=0x2000) error(2, "Address out of bounds");
					write1(0xCA);
