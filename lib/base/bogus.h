...
generate_fixed_string(string, PATH_MAX);
...
generate_queue(task_queue, queue_item_t);




void init() {
    string_from_c_string(&tool_paths.gcc_path, "/usr/bin/gcc -Wl,-fuse-ld=gold");
    string_from_c_string(&tool_paths.gpp_path, "/usr/bin/g++");
    string_from_c_string(&tool_paths.clang_path, "/usr/bin/clang");
    string_from_c_string(&tool_paths.clangpp_path, "/usr/bin/clang++");
    string_from_c_string(&tool_paths.emcc_path, "/home/horrowind/Projects/emscripten/emscripten/master/emcc");
    string_from_c_string(&tool_paths.empp_path, "/home/horrowind/Projects/emscripten/emscripten/master/em++");
    string_from_c_string(&tool_paths.mxe_gcc_path, "/home/horrowind/Projects/mxe/usr/bin/i686-w64-mingw32.static-gcc");
    string_from_c_string(&tool_paths.mxe_gpp_path, "/home/horrowind/Projects/mxe/usr/bin/i686-w64-mingw32.static-g++");
    string_from_c_string(&tool_paths.ar_path, "/usr/bin/ar");
    string_from_c_string(&tool_paths.mxe_ar_path, "/home/horrowind/Projects/mxe/usr/bin/i686-w64-mingw32.static-ar");
...
target_t target_init(string_t name, target_type_t type) {
    target_t target;
    string_from_c_string(target.name, name, sizeof(string_t));
    string_from_c_string(target.file, "", sizeof(string_t));
    string_from_c_string(target.output_dir, "", sizeof(string_t));
    target.type = type;
    target.objects = NULL;
...
void target_add_library(target_t* target, string_t library) {
    ...
    string_from_c_string(target->libraries[target->libraries_length], library, sizeof(string_t));
...
void target_add_libraries(target_t* target, string_t* libraries, int length) {
    ...
    for(int i = 0; i < length; i++) {
        string_from_c_string(target->libraries[target->libraries_length + i], libraries[i], sizeof(string_t));
    }
...
void target_add_library_path(target_t* target, string_t paths) {
    ...
    string_from_c_string(target->library_paths[target->library_paths_length], paths, sizeof(string_t));
...
void target_add_library_paths(target_t* target, string_t* paths, int length) {
    ...
    for(int i = 0; i < length; i++) {
        string_from_c_string(target->library_paths[target->library_paths_length + i], paths[i], sizeof(string_t));
    }
...
void target_add_object(target_t* target, object_t object) {
    ...
    string_from_c_string(target->objects[target->objects_length].file, object.file, sizeof(string_t));
...
void target_add_objects(target_t* target, object_t* objects, int length) {
    ...
    for(int i = 0; i < length; i++) {
        string_from_c_string(target->objects[target->objects_length + i].file, objects[i].file, sizeof(string_t));
    }
