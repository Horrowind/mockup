...
struct DockContext
{
        ...
        for (int i = 0; i < m_docks.size(); ++i)
        {
            ...
            else
            {
                ...
                if (dock.status == Status_Dragged) dsize.y = io.MouseDelta.y;
                        ...
                        ImVec2 new_size0( dock.children[0]->size.x + dsize.x, dock.children[0]->size.y + dsize.y );
                        ImVec2 new_size1( dock.children[1]->size.x - dsize.x, dock.children[1]->size.y - dsize.y );
                        ImVec2 new_pos1( dock.children[1]->pos.x + dsize.x, dock.children[1]->pos.y + dsize.y );
    ...
    Dock* getDockAt(const ImVec2& pos) const
    {
        for (int i = 0; i < m_docks.size(); ++i)
        {
            ...
            if (IsMouseHoveringRect(dock.pos, ImVec2(dock.pos.x + dock.size.x, dock.pos.y + dock.size.y), false))
            {
                return &dock;
            }
        }
    ...
    static ImRect getDockedRect(const ImRect& rect, Slot_ dock_slot)
    {
        ...
        {
            default: return rect;
            case Slot_Top: return ImRect(rect.Min, ImVec2(rect.Min.x + rect.Max.x, rect.Min.y + half_size.y));
            case Slot_Right:
                return ImRect( ImVec2(rect.Min.x+half_size.x, rect.Min.y + 0), rect.Max);
            case Slot_Bottom:
                return ImRect( ImVec2(rect.Min.x+0, rect.Min.y + half_size.y), rect.Max);
            case Slot_Left: return ImRect(rect.Min, ImVec2(rect.Min.x + half_size.x, rect.Min.y + rect.Max.y));
        }
    }
    ...
    {
        ImVec2 size( parent_rect.Max.x - parent_rect.Min.x, parent_rect.Max.y - parent_rect.Min.y );
        ImVec2 center( parent_rect.Min.x + size.x * 0.5f, parent_rect.Min.y + size.y * 0.5f );
        switch (dock_slot)
        {
            default: return ImRect(ImVec2(center.x - 20, center.y - 20), ImVec2(center.x + 20, center.y + 20));
            case Slot_Top: return ImRect(ImVec2(center.x + -20, center.y + -50), ImVec2(center.x + 20, center.y + -30));
            case Slot_Right: return ImRect(ImVec2(center.x + 30, center.y + -20), ImVec2(center.x + 50, center.y + 20));
            case Slot_Bottom: return ImRect(ImVec2(center.x + -20, center.y + +30), ImVec2(center.x + 20, center.y + 50));
            case Slot_Left: return ImRect(ImVec2(center.x + -50, center.y + -20), ImVec2(center.x + -30, center.y + 20));
        }
    }
    ...
    void handleDrag(Dock& dock)
    {
        ...
        dock.pos = ImVec2( mp.x - m_drag_offset.x, mp.y - m_drag_offset.y );
        if (dest_dock)
        {
            if (dockSlots(dock, dest_dock, ImRect(dest_dock->pos, ImVec2(dest_dock->pos.x + dest_dock->size.x, dest_dock->pos.y + dest_dock->size.y)), false))
            {
                canvas->PopClipRect();
                End();
                return;
        ...
        canvas->AddRectFilled(dock.pos, ImVec2(dock.pos.x + dock.size.x, dock.pos.y + dock.size.y), docked_color);
        canvas->PopClipRect();
        ...
        if (BeginChild(tmp, size, true))
        {
            ...
            while (dock_tab)
            {
                ...
                if (IsItemActive() && IsMouseDragging())
                {
                    ...
                    m_drag_offset = ImVec2( mp.x - dock_tab->pos.x, mp.y - dock_tab->pos.y );
                    doUndock(*dock_tab);
                ...
                if (dock_tab->active && close_button)
                {
                    ...
                    draw_list->AddLine(ImVec2(center.x - 3.5f, center.y - 3.5f), ImVec2(center.x + 3.5f, center.y + 3.5f), text_color);
                    draw_list->AddLine(ImVec2(center.x + 3.5f, center.y - 3.5f), ImVec2(center.x - 3.5f, center.y + 3.5f), text_color);
                }
                tab_base = pos.y;
                draw_list->PathClear();
                draw_list->PathLineTo(ImVec2(pos.x+-15, pos.y+size.y));
                draw_list->PathBezierCurveTo(
                    ImVec2(pos.x-10, pos.y+size.y), ImVec2(pos.x-5, pos.y+0), ImVec2(pos.x+0, pos.y+0), 10);
                draw_list->PathLineTo(ImVec2(pos.x+size.x, pos.y+0));
                draw_list->PathBezierCurveTo(ImVec2(pos.x+size.x + 5, pos.y+0),
                    ImVec2(pos.x+size.x + 10, pos.y+size.y),
                    ImVec2(pos.x+size.x + 15, pos.y+size.y),
                    10);
                draw_list->PathFill(
            ...
            draw_list->AddLine(cp, ImVec2(cp.x+dock.size.x, cp.y+0), color);
        }
        EndChild();
        return tab_closed;
    }
    ...
    bool begin(const char* label, bool* opened, ImGuiWindowFlags extra_flags)
    {
        ...
        if (is_float)
        {
            ...
            {
                ImVec2 mp = GetMousePos();
                m_drag_offset = ImVec2( mp.x - dock.pos.x, mp.y - dock.pos.y );
                doUndock(dock);
                dock.status = Status_Dragged;
            }
            return ret;
        ...
        if (tabbar(dock.getFirstTab(), opened != 0))
        {
            *opened = false;
        }
        ImVec2 pos = dock.pos;
